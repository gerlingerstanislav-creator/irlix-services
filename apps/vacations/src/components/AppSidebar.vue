...
<template>
  <aside class="app-sidebar" :class="{ 'services-open': showServices }">
    ...
    <div class="nav-hover-zone">
      <nav class="icon-nav" aria-label="Навигация сервиса отпусков">
        <button v-for="item in items" :key="item.id" type="button" :class="{ active: props.section === item.id }" :aria-label="item.label" @click="go(item.id)">
          <svg v-if="item.icon === 'calendar'" viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="5" width="16" height="15" rx="2"/><path d="M8 3v4M16 3v4M4 10h16"/></svg>
          <svg v-else-if="item.icon === 'team'" viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><path d="M3.5 19c.4-4 2.4-6 5.5-6s5.1 2 5.5 6"/><circle cx="17" cy="9" r="2.3"/><path d="M15.5 14.2c3.3-.6 5 1.1 5.5 4.3"/></svg>
          <svg v-else-if="item.icon === 'manage'" viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M8 9h8M8 13h8M8 17h5"/></svg>
        </button>
      </nav>
      <div class="nav-labels">
        <button v-for="item in items" :key="item.id" type="button" :class="{ active: props.section === item.id }" @click="go(item.id)">{{ item.label }}</button>
      </div>
    </div>

    <div class="sidebar-bottom" aria-label="Системные действия">
      <button v-if="canReadHistory" class="audit-link" type="button" :class="{ active: props.section === 'history' }" aria-label="История действий" title="История действий" @click="go('history')">
    ...
    <div v-if="showServices" ref="servicesPopover" class="services-popover">
      ...
      <div class="services-list">
        <button v-for="service in serviceItems" :key="service.key" type="button" :disabled="!service.available" :class="{ active: service.key === 'vacations' }" @click="openService(service)">
...
.nav-labels button { height: 30px; width: max-content; min-width: 92px; max-width: 180px; padding: 0 9px; border: 0; border-radius: 7px; background: rgba(67,69,72,.72); color: #fff; text-align: left; font-size: 12px; font-weight: 650; cursor: pointer; box-shadow: 0 3px 10px rgba(19,25,32,.10); white-space: nowrap; }
