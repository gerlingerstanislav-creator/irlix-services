  ...
  <main class="page">
    <section class="hero">
      <div>
        ...
        <p>
          Первый стенд новой платформы. Общая оболочка и независимые backend-сервисы
          уже запускаются через единый Docker-контур.
        </p>
      </div>

      <button type="button" @click="refresh">Проверить сервисы</button>
    </section>
    ...
    <section class="grid">
      <article v-for="service in services" :key="service.key" class="card">
        ...
        <h2>{{ service.name }}</h2>
        <p v-if="service.key === 'platform'">
          Аутентификация, permissions, каталог сервисов и общие платформенные механизмы.
        </p>
        <p v-else>
          Сотрудники, оргструктура и жизненный цикл сотрудника. Первый эталонный бизнес-сервис.
        </p>
      </article>
    </section>
  </main>
