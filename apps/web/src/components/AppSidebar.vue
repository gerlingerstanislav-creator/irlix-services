...
<template>
  <aside class="app-sidebar" :class="{ 'services-open': showServices }">
    <button class="services-logo" type="button" aria-label="Открыть список сервисов" :aria-expanded="showServices" @click="showServices = !showServices">
    ...
    <div class="nav-labels" aria-hidden="false">
    ...
    <div v-if="showServices" class="services-popover">
      <div class="services-popover__header">
        <strong>Сервисы</strong>
        <button type="button" aria-label="Закрыть" @click="showServices = false">×</button>
      </div>
...
.logo-one { fill: #15191f; }
.logo-two { fill: var(--irlix-color-primary); }
.sidebar-divider { width: 100%; height: 1px; background: #eceef0; }
.icon-nav { width: 100%; display: grid; justify-items: center; gap: 7px; padding-top: 12px; }
.icon-nav button,
...
.nav-labels button {
  ...
  border: 0;
  border-radius: 8px;
  background: rgba(67, 69, 72, .70);
  color: #fff;
  text-align: left;
  font-size: 13px;
  font-weight: 650;
  cursor: pointer;
  box-shadow: 0 4px 14px rgba(19, 25, 32, .10);
}
.nav-labels button:hover { background: rgba(45, 47, 50, .84); }
...
.sidebar-bottom button:nth-child(2) { color: #d07013; background: #fff0e2; }
.sidebar-bottom button:hover { background: #f3f5f5; }
.services-popover {
  position: absolute;
  top: 8px;
  left: 76px;
  ...
  border-radius: 12px;
  background: #fff;
  box-shadow: 0 12px 34px rgba(20, 27, 38, .14);
}
.services-popover__header { display: flex; align-items: center; justify-content: space-between; padding: 5px 6px 8px 10px; }
.services-popover__header strong { font-size: 14px; }
.services-popover__header button { width: 28px; height: 28px; padding: 0; border: 0; border-radius: 6px; background: transparent; color: #858c95; font-size: 20px; cursor: pointer; }
.services-popover__header button:hover { background: #f3f4f5; }
...
.services-list > button:hover:not(:disabled) { background: #f3f8f6; }
.services-list > button.active { background: #eef9f5; }
.services-list > button:disabled { cursor: default; opacity: .5; }
...
.service-copy { display: grid; gap: 2px; }
.service-copy strong { font-size: 13px; }
.service-copy small { color: #8b929b; font-size: 11px; font-weight: 400; }
@media (max-width: 720px) {
  .app-sidebar { position: static; width: 100%; height: auto; flex-direction: row; padding: 6px 8px; border-right: 0; border-bottom: 1px solid #e1e4e8; }
  .sidebar-divider { display: none; }
  .icon-nav { width: auto; display: flex; padding: 0; margin-left: 6px; gap: 3px; }
